 ...
 * # UByte Deque Base
 *
 * Base, templated type backing the `UByteDeque` class.
 *
...
abstract class AbstractUByteDeque<D : AbstractUByteDeque<D>> {

  protected var container: UByteArray
  ...
  constructor(initCapacity: Int = 0) {
    container = UByteArray(initCapacity)
    size = 0
    realHead = 0
  }

  constructor(values: UByteArray) {
    container = values.copyOf()
    size = values.size
    realHead = 0
  }

  protected constructor(values: UByteArray, head: Int) {
    container = values
    size = values.size
    realHead = head
  }
   ...
   */
  protected abstract fun new(values: UByteArray, head: Int): D
   ...
   */
  fun copyInto(array: UByteArray, offset: Int = 0, len: Int = array.size - offset): Int {
   ...
   * greater than [size], or if [start] is greater than [end].
   */
  fun sliceToArray(start: Int, end: Int): UByteArray {
   ...
   */
  operator fun plus(rhs: AbstractUByteDeque<*>) =
    new(UByteArray(size + rhs.size).also { copyInto(it); rhs.copyInto(it, size) }, 0)
   ...
   */
  inline fun firstOr(value: UByte) = frontOr(value)
   ...
   */
  inline fun lastOr(value: UByte) = backOr(value)
   ...
   */
  fun iterator() = object : ListIterator<UByte> {
    var position = -1
   ...
   */
  inline fun pushFirst(value: UByte) = pushFront(value)
   ...
   */
  fun pushBack(values: UByteArray) {
   ...
   */
  fun pushBack(values: Collection<UByte>) {
   ...
   */
  fun pushBack(values: AbstractUByteDeque<*>) {
   ...
   */
  inline fun pushLast(value: UByte) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: UByteArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: Collection<UByte>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: AbstractUByteDeque<*>) = pushBack(values)
   ...
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(value: UByte) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: UByteArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: Collection<UByte>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: AbstractUByteDeque<*>) = pushBack(values)
   ...
   * is greater than [lastIndex].
   */
  operator fun set(index: Int, value: UByte) = container.set(vei(index), value)
   ...
   */
  fun insert(index: Int, value: UByte) {
   ...
   * deque was empty.
   */
  fun popFrontOr(value: UByte): UByte {
    if (size == 0)
      return value
   ...
   */
  inline fun popFirstOr(value: UByte) = popFrontOr(value)
   ...
   * deque was empty.
   */
  fun popBackOr(value: UByte): UByte {
    if (size == 0)
      return value
   ...
   */
  inline fun popLastOr(value: UByte) = popBackOr(value)
  ...
  override fun toString() = "UByteDeque($size/${container.size})"

  override fun equals(other: Any?) =
    if (other is AbstractUByteDeque<*>)
