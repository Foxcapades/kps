 ...
 * # Float Deque Base
 *
 * Base, templated type backing the `FloatDeque` class.
 *
...
abstract class AbstractFloatDeque<D : AbstractFloatDeque<D>> {

  protected var container: FloatArray
  ...
  constructor(initCapacity: Int = 0) {
    container = FloatArray(initCapacity)
    size = 0
    realHead = 0
  }
  ...
  protected constructor(values: FloatArray, head: Int) {
    container = values
    size = values.size
    realHead = head
  }
   ...
   */
  protected abstract fun new(values: FloatArray, head: Int): D
  ...
  fun ensureCapacity(minCapacity: Int) {
    when {
      ...
      container.isEmpty()           -> container = FloatArray(minCapacity)
   ...
   */
  fun copyInto(array: FloatArray, offset: Int = 0, len: Int = array.size - offset): Int {
   ...
   * greater than [size], or if [start] is greater than [end].
   */
  fun sliceToArray(start: Int, end: Int): FloatArray {
    ...
    when (realSize) {
      ...
      1    -> return FloatArray(1) { container[internalIndex(start)] }
      size -> return toArray()
    }
   ...
   */
  operator fun plus(rhs: AbstractFloatDeque<*>) =
    new(FloatArray(size + rhs.size).also { copyInto(it); rhs.copyInto(it, size) }, 0)
   ...
   */
  inline fun firstOr(value: Float) = frontOr(value)
   ...
   */
  inline fun lastOr(value: Float) = backOr(value)
   ...
   */
  inline fun pushFirst(value: Float) = pushFront(value)
   ...
   */
  fun pushBack(values: FloatArray) {
   ...
   */
  inline fun pushLast(value: Float) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: FloatArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: Collection<Float>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: AbstractFloatDeque<*>) = pushBack(values)
   ...
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(value: Float) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: FloatArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: Collection<Float>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: AbstractFloatDeque<*>) = pushBack(values)
   ...
   * deque was empty.
   */
  fun popBackOr(value: Float): Float {
    if (size == 0)
      return value
   ...
   */
  inline fun popLastOr(value: Float) = popBackOr(value)
  ...
  override fun toString() = "FloatDeque($size/${container.size})"

  override fun equals(other: Any?) =
    if (other is AbstractFloatDeque<*>)
