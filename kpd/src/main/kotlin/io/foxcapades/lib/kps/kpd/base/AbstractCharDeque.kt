 ...
 * # Char Deque Base
 *
 * Base, templated type backing the `CharDeque` class.
 *
...
abstract class AbstractCharDeque<D : AbstractCharDeque<D>> {

  protected var container: CharArray
  ...
  constructor(initCapacity: Int = 0) {
    container = CharArray(initCapacity)
    size = 0
    realHead = 0
  }

  constructor(values: CharArray) {
    container = values.copyOf()
    size = values.size
    realHead = 0
  }

  protected constructor(values: CharArray, head: Int) {
    container = values
    size = values.size
    realHead = head
  }
   ...
   */
  protected abstract fun new(values: CharArray, head: Int): D
   ...
   */
  fun copyInto(array: CharArray, offset: Int = 0, len: Int = array.size - offset): Int {
   ...
   * greater than [size], or if [start] is greater than [end].
   */
  fun sliceToArray(start: Int, end: Int): CharArray {
   ...
   */
  operator fun plus(rhs: AbstractCharDeque<*>) =
    new(CharArray(size + rhs.size).also { copyInto(it); rhs.copyInto(it, size) }, 0)
   ...
   */
  inline fun firstOr(value: Char) = frontOr(value)
   ...
   */
  inline fun lastOr(value: Char) = backOr(value)
   ...
   */
  fun iterator() = object : ListIterator<Char> {
    var position = -1
   ...
   */
  inline fun pushFirst(value: Char) = pushFront(value)
   ...
   */
  fun pushBack(values: Collection<Char>) {
   ...
   */
  fun pushBack(values: AbstractCharDeque<*>) {
   ...
   */
  inline fun pushLast(value: Char) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: CharArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: Collection<Char>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: AbstractCharDeque<*>) = pushBack(values)
   ...
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(value: Char) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: CharArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: Collection<Char>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: AbstractCharDeque<*>) = pushBack(values)
   ...
   * is greater than [lastIndex].
   */
  operator fun set(index: Int, value: Char) = container.set(vei(index), value)
   ...
   * deque was empty.
   */
  fun popFrontOr(value: Char): Char {
    if (size == 0)
      return value
   ...
   */
  inline fun popFirstOr(value: Char) = popFrontOr(value)
  ...
  override fun toString() = "CharDeque($size/${container.size})"

  override fun equals(other: Any?) =
    if (other is AbstractCharDeque<*>)
