 ...
 * # UInt Deque Base
 *
 * Base, templated type backing the `UIntDeque` class.
 *
...
abstract class AbstractUIntDeque<D : AbstractUIntDeque<D>> {

  protected var container: UIntArray
  ...
  constructor(initCapacity: Int = 0) {
    container = UIntArray(initCapacity)
    size = 0
    realHead = 0
  }

  constructor(values: UIntArray) {
    container = values.copyOf()
    size = values.size
    realHead = 0
  }

  protected constructor(values: UIntArray, head: Int) {
    container = values
    size = values.size
    realHead = head
  }
   ...
   */
  protected abstract fun new(values: UIntArray, head: Int): D
  ...
  fun ensureCapacity(minCapacity: Int) {
    when {
      ...
      container.isEmpty()           -> container = UIntArray(minCapacity)
   ...
   */
  fun copyInto(array: UIntArray, offset: Int = 0, len: Int = array.size - offset): Int {
   ...
   * greater than [size], or if [start] is greater than [end].
   */
  fun sliceToArray(start: Int, end: Int): UIntArray {
    ...
    // Shortcuts
    when (realSize) {
      0    -> return UIntArray(0)
      1    -> return UIntArray(1) { container[internalIndex(start)] }
      size -> return toArray()
    }
   ...
   */
  operator fun plus(rhs: AbstractUIntDeque<*>) =
    new(UIntArray(size + rhs.size).also { copyInto(it); rhs.copyInto(it, size) }, 0)
   ...
   */
  inline fun firstOr(value: UInt) = frontOr(value)
   ...
   */
  inline fun lastOr(value: UInt) = backOr(value)
   ...
   */
  fun iterator() = object : ListIterator<UInt> {
    var position = -1
   ...
   */
  inline fun pushFirst(value: UInt) = pushFront(value)
   ...
   */
  fun pushBack(values: Collection<UInt>) {
   ...
   */
  fun pushBack(values: AbstractUIntDeque<*>) {
   ...
   */
  inline fun pushLast(value: UInt) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: UIntArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: Collection<UInt>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: AbstractUIntDeque<*>) = pushBack(values)
   ...
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(value: UInt) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: UIntArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: Collection<UInt>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: AbstractUIntDeque<*>) = pushBack(values)
   ...
   * is greater than [lastIndex].
   */
  operator fun set(index: Int, value: UInt) = container.set(vei(index), value)
   ...
   * deque was empty.
   */
  fun popFrontOr(value: UInt): UInt {
    if (size == 0)
      return value
   ...
   */
  inline fun popFirstOr(value: UInt) = popFrontOr(value)
   ...
   */
  inline fun popLastOr(value: UInt) = popBackOr(value)
  ...
  override fun toString() = "UIntDeque($size/${container.size})"

  override fun equals(other: Any?) =
    if (other is AbstractUIntDeque<*>)
