 ...
 * # Short Deque Base
 *
 * Base, templated type backing the `ShortDeque` class.
 *
...
abstract class AbstractShortDeque<D : AbstractShortDeque<D>> {

  protected var container: ShortArray
  ...
  constructor(initCapacity: Int = 0) {
    container = ShortArray(initCapacity)
    size = 0
    realHead = 0
  }

  constructor(values: ShortArray) {
    container = values.copyOf()
    size = values.size
    realHead = 0
  }

  protected constructor(values: ShortArray, head: Int) {
    container = values
    size = values.size
    realHead = head
  }
   ...
   */
  protected abstract fun new(values: ShortArray, head: Int): D
  ...
  fun ensureCapacity(minCapacity: Int) {
    when {
      ...
      container.isEmpty()           -> container = ShortArray(minCapacity)
   ...
   */
  fun copyInto(array: ShortArray, offset: Int = 0, len: Int = array.size - offset): Int {
   ...
   * greater than [size], or if [start] is greater than [end].
   */
  fun sliceToArray(start: Int, end: Int): ShortArray {
    ...
    // Shortcuts
    when (realSize) {
      0    -> return ShortArray(0)
      1    -> return ShortArray(1) { container[internalIndex(start)] }
      size -> return toArray()
    }
   ...
   */
  operator fun plus(rhs: AbstractShortDeque<*>) =
    new(ShortArray(size + rhs.size).also { copyInto(it); rhs.copyInto(it, size) }, 0)
   ...
   */
  inline fun firstOr(value: Short) = frontOr(value)
   ...
   */
  inline fun lastOr(value: Short) = backOr(value)
   ...
   */
  fun iterator() = object : ListIterator<Short> {
    var position = -1
   ...
   */
  inline fun pushFirst(value: Short) = pushFront(value)
   ...
   */
  fun pushBack(values: ShortArray) {
   ...
   */
  fun pushBack(values: Collection<Short>) {
   ...
   */
  fun pushBack(values: AbstractShortDeque<*>) {
   ...
   */
  inline fun pushLast(value: Short) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: ShortArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: Collection<Short>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: AbstractShortDeque<*>) = pushBack(values)
   ...
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(value: Short) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: ShortArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: Collection<Short>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: AbstractShortDeque<*>) = pushBack(values)
   ...
   * is greater than [lastIndex].
   */
  operator fun set(index: Int, value: Short) = container.set(vei(index), value)
   ...
   * deque was empty.
   */
  fun popFrontOr(value: Short): Short {
    if (size == 0)
      return value
   ...
   */
  inline fun popFirstOr(value: Short) = popFrontOr(value)
   ...
   * deque was empty.
   */
  fun popBackOr(value: Short): Short {
    if (size == 0)
      return value
   ...
   */
  inline fun popLastOr(value: Short) = popBackOr(value)
  ...
  override fun toString() = "ShortDeque($size/${container.size})"

  override fun equals(other: Any?) =
    if (other is AbstractShortDeque<*>)
