 ...
 * # Boolean Deque Base
 *
 * Base, templated type backing the `BooleanDeque` class.
 *
...
abstract class AbstractBooleanDeque<D : AbstractBooleanDeque<D>> {

  protected var container: BooleanArray
  ...
  constructor(initCapacity: Int = 0) {
    container = BooleanArray(initCapacity)
    size = 0
    realHead = 0
  }
  ...
  protected constructor(values: BooleanArray, head: Int) {
    container = values
    size = values.size
    realHead = head
  }
   ...
   */
  protected abstract fun new(values: BooleanArray, head: Int): D
  ...
  fun ensureCapacity(minCapacity: Int) {
    when {
      ...
      container.isEmpty()           -> container = BooleanArray(minCapacity)
   ...
   */
  fun copyInto(array: BooleanArray, offset: Int = 0, len: Int = array.size - offset): Int {
   ...
   * greater than [size], or if [start] is greater than [end].
   */
  fun sliceToArray(start: Int, end: Int): BooleanArray {
    ...
    // Shortcuts
    when (realSize) {
      0    -> return BooleanArray(0)
      1    -> return BooleanArray(1) { container[internalIndex(start)] }
      size -> return toArray()
    }
   ...
   */
  operator fun plus(rhs: AbstractBooleanDeque<*>) =
    new(BooleanArray(size + rhs.size).also { copyInto(it); rhs.copyInto(it, size) }, 0)
   ...
   */
  inline fun firstOr(value: Boolean) = frontOr(value)
   ...
   */
  inline fun lastOr(value: Boolean) = backOr(value)
   ...
   */
  fun iterator() = object : ListIterator<Boolean> {
    var position = -1
   ...
   */
  inline fun pushFirst(value: Boolean) = pushFront(value)
   ...
   */
  fun pushBack(values: BooleanArray) {
   ...
   */
  fun pushBack(values: Collection<Boolean>) {
   ...
   */
  fun pushBack(values: AbstractBooleanDeque<*>) {
   ...
   */
  inline fun pushLast(value: Boolean) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: BooleanArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: Collection<Boolean>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline fun pushLast(values: AbstractBooleanDeque<*>) = pushBack(values)
   ...
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(value: Boolean) = pushBack(value)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: BooleanArray) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: Collection<Boolean>) = pushBack(values)

  /**
   * Inline alias of [pushBack]
   */
  inline operator fun plusAssign(values: AbstractBooleanDeque<*>) = pushBack(values)
   ...
   * is greater than [lastIndex].
   */
  operator fun set(index: Int, value: Boolean) = container.set(vei(index), value)
   ...
   * deque was empty.
   */
  fun popFrontOr(value: Boolean): Boolean {
    if (size == 0)
      return value
   ...
   */
  inline fun popFirstOr(value: Boolean) = popFrontOr(value)
   ...
   * deque was empty.
   */
  fun popBackOr(value: Boolean): Boolean {
    if (size == 0)
      return value
   ...
   */
  inline fun popLastOr(value: Boolean) = popBackOr(value)
  ...
  override fun toString() = "BooleanDeque($size/${container.size})"

  override fun equals(other: Any?) =
    if (other is AbstractBooleanDeque<*>)
